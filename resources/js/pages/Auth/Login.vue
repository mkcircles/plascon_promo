    ...
    <div class="p-8 sm:p-10">
        ...
        <div class="text-center mb-8">
            <div class="mb-5 flex justify-center">
                <img src="/images/logo.png" alt="Plascon Uganda" class="w-44 h-auto" />
            </div>
            <h2 class="text-2xl font-bold tracking-tight text-slate-800">Plascon Promo Portal</h2>
            <p v-if="mode === 'login'" class="text-sm text-slate-600 mt-1">Sign in to the Plascon administration panel
            </p>
        ...
        <div v-if="successMessage" class="mb-6 transform translate-y-0 transition-all duration-300">
            <div class="flex items-center bg-emerald-50/80 border border-emerald-200/50 rounded-xl p-4 text-sm text-emerald-600 backdrop-blur-sm"
                role="alert">
                <svg class="w-5 h-5 inline mr-3 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"
        ...
        <div v-if="authError" class="mb-6 transform translate-y-0 transition-all duration-300">
            <div class="flex items-center bg-red-50/80 border border-red-200/50 rounded-xl p-4 text-sm text-red-600 backdrop-blur-sm"
                role="alert">
                <svg class="w-5 h-5 inline mr-3 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"
                    xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd"
        ...
        <form v-if="mode === 'login'" @submit.prevent="login" class="space-y-5">
            <div>
                <label class="block text-xs font-semibold uppercase tracking-wider text-slate-600 mb-2 px-1">Email
                    Address</label>
                <div class="relative">
                    ...
                    <input type="email" v-model="user.username" required placeholder="Enter your email"
            ...
            <div>
                <label
                    class="block text-xs font-semibold uppercase tracking-wider text-slate-600 mb-2 px-1">Password</label>
                <div class="relative">
                    <span class="absolute inset-y-0 left-0 pl-3.5 flex items-center text-slate-400">
                        <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <input type="password" v-model="user.password" required placeholder="••••••••"
                        class="text-sm block pl-11 pr-4 py-3 rounded-xl w-full bg-white/60 border border-slate-300/60 placeholder-slate-400 text-slate-800 shadow-sm focus:placeholder-slate-300 focus:bg-white focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 focus:outline-none transition-all duration-200" />
                </div>
            </div>
            ...
            <div class="flex items-center justify-between text-xs pt-1">
                ...
                <a href="#" @click.prevent="switchMode('forgot')"
                    class="font-semibold text-blue-600 hover:text-blue-700 transition duration-150">
                    Forgot Password?
                </a>
            </div>
            ...
            <div class="pt-2">
                <button type="submit"
                    ...
                    <div
                        class="absolute inset-0 -translate-x-full group-hover:translate-x-full bg-gradient-to-r from-transparent via-white/20 to-transparent transition-transform duration-1000 ease-in-out">
                    </div>
                </button>
            </div>
        </form>
        ...
        <form v-else-if="mode === 'forgot'" @submit.prevent="sendResetCode" class="space-y-5">
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="email" v-model="forgotEmail" required placeholder="Enter your registered email"
                        class="text-sm block pl-11 pr-4 py-3 rounded-xl w-full bg-white/60 border border-slate-300/60 placeholder-slate-400 text-slate-800 shadow-sm focus:placeholder-slate-300 focus:bg-white focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 focus:outline-none transition-all duration-200" />
                </div>
            </div>
        ...
        <form v-else-if="mode === 'reset'" @submit.prevent="resetPassword" class="space-y-5">
            <div>
                <label
                    class="block text-xs font-semibold uppercase tracking-wider text-slate-600 mb-2 px-1">Verification
                    Code</label>
                <div class="relative">
                    <span class="absolute inset-y-0 left-0 pl-3.5 flex items-center text-slate-400">
                        <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    ...
                    <input type="text" v-model="resetToken" required placeholder="Enter the 6-digit code"
                        class="text-sm block pl-11 pr-4 py-3 rounded-xl w-full bg-white/60 border border-slate-300/60 placeholder-slate-400 text-slate-800 shadow-sm focus:placeholder-slate-300 focus:bg-white focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 focus:outline-none transition-all duration-200" />
                </div>
            </div>

            <div>
                <label class="block text-xs font-semibold uppercase tracking-wider text-slate-600 mb-2 px-1">New
                    Password</label>
                <div class="relative">
                    <span class="absolute inset-y-0 left-0 pl-3.5 flex items-center text-slate-400">
                        <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
