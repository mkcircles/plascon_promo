        ...
        <div class="mb-4 flex justify-between items-center">
            <div class="flex-1 pr-4">
                <div class="relative md:w-1/3">
                    <h2 class="text-2xl font-semibold leading-normal mt-0 text-gray-800" id="basic">{{areaName}} Codes</h2>
...
export default {
    ...
    created() {
        this.area = this.$route.path.split('/').pop();
        this.getEntries();

    },
    beforeRouteUpdate(to, from, next) {
        this.area = to.path.split('/').pop();
        this.getEntries();
        next();
    },
    computed: {
        //Replace _ with space and capitalize first letter in area
        areaName() {
            return this.area.replace(/_/g, " ").replace(/\w\S*/g, function (txt) {
    ...
    methods: {
        async getEntries(page = 1) {
            const response = await axios.get("/api/area-codes/"+this.area+"?page=" + page, {
                headers: {
