<template>
        ...
        <div class="mb-4 flex justify-between items-center">
            <div class="flex-1 pr-4">
                <div class="relative md:w-1/3">
                    <h2 class="text-2xl font-semibold leading-normal mt-0 text-gray-800" id="basic">Sent Airtime</h2>
                    <p class="text-gray-500">All Sent Out Airtime</p>
                </div>
            </div>
            <div>
                <form>   
    ...
    <div class="relative">
        ...
        <button type="submit" @click.prevent="searchPhoneAirtime" class="text-white absolute right-2.5 bottom-2.5 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Search</button>
    </div>
</form>
            </div>
        </div>
        ...
        <TableData :colums="columns" >
                ...
                >
                    ...
                    <td class="py-2 px-6 text-left">{{ td.amount }}</td>
...
export default {
    ...
    data() {
        return {
            columns: [
                ...
                { name: "Response", key: "Message", sortable: true },
            ],
            entries: [],
            phoneNumber: "",
            links: [],
    ...
    methods: {
        async getEntries(page = 1) {
            const response = await axios.get("/api/airtime?page=" + page, {
                headers: {
        ...
        fieldState(status) {
            if (status === "pending") {
                ...
                return "bg-teal-200 text-teal-600 py-1 px-3 rounded-full text-xs capitalize";
        ...
        searchPhoneAirtime() {
            if (this.phoneNumber.length > 0) {
                const response = axios.get("/api/airtime/search/" + this.phoneNumber, {
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem("token")}`,
                    },
                });
