<template>
  <div class="bg-white rounded-3xl shadow-xl p-6 border border-slate-100 transition-all duration-300">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
      <div>
        <h3 class="text-lg font-bold text-slate-800">Inbound SMS Traffic</h3>
        <p class="text-sm text-slate-500">Track total daily incoming SMS requests</p>
      </div>
...
const fetchData = async () => {
  loading.value = true;
  try {
    const res = await axios.get(`/api/chart?days=${days.value}`, {
      headers: {
    ...
    console.error("Error fetching message traffic data:", error);
  } finally {
    loading.value = false;
  }
};
